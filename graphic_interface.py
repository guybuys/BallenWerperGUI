...
class Slider:
    ...
    def handle_event(self, event):
        if event.type == MOUSEBUTTONDOWN:
            ...
            if event.key == K_EQUALS and self.is_mouse_over():
                self.increase_value()
            elif event.key == K_MINUS and self.is_mouse_over():
                self.decrease_value()
    ...
    def increase_value(self):
        # Increase the value by 1 if within range
        new_value = min(self.max_value, self.value + 1)
        if new_value != self.value:
            self.update_value(new_value)
            self.update_slider_position()

    def decrease_value(self):
        # Decrease the value by 1 if within range
        new_value = max(self.min_value, self.value - 1)
        if new_value != self.value:
            self.update_value(new_value)
            self.update_slider_position()
